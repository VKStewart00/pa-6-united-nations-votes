...
```{r}
#|label: G7-voting-graph

unvotes |> 
  count(country, vote) |> #total count of votes per country
  group_by(country) |> #separate by countries
  mutate(total = sum(n)) |> #adding column with total counts 
  mutate(prop_vote = n/total) |> #adding column with proportional votes
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #keeps the listed countries
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #creates a plot with country on x axis and y as proportions
  geom_col(position = position_stack()) + #creates stacked bar graph
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #labels axis and graph title
  theme_minimal() +  #specifies theme
  scale_fill_viridis_d(end = 0.8) + #uses viridis color scale which ends at a specific point
  coord_flip() #flips axis
```

> Describe what the graph above demonstrates above UN voting records for the G7 
> The United Sattes had the most no's and the France had the most abstain. Germany said yes the most. 
...
```{r}
#|label: Roll call unique inputs
roll_calls |> 
  distinct(short)
```

> Unique inputs of short descriptions on what UN members are voting on.


We can use the individual data for `roll_calls` to look at the number of votes per year over time.

```{r}
#|label: Figure for roll call data
...
> Separate by country, and adding if the voting was deemed important by the U.S. State Department.


We could try and look at the short descriptions, `short`, for each vote with the following code. 
```{r}
#|label: Adds counts to roll_call data
...
> The amount of times an issue has been repeatedly voted on.  
...
> Roll_calls df has more number of rows compared to isses df. This means we may lose or add more rows based on how we join our data. 
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #matching the rcid column from both df; more rows
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #matching the rcid column from both df; less rows
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #keeps all the rows (whether it matches or not)and adds more rows
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #only keeps rows that match; less rows
```
...
> There was a spike in the 1960s, 80s, and early 2000s.  
